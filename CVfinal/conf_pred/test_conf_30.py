...
def main(config):
    ...
    for batch in test_loader:
        ...
        if img_paths.split('/')[-2] not in predictions:
            predictions[img_paths.split('/')[-2]] = logits.argmax(dim=-1).cpu().numpy().tolist()
        else:
            predictions[img_paths.split('/')[-2]].extend(logits.argmax(dim=-1).cpu().numpy().tolist())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./conf_pred/0604_s5.json')
    parser.add_argument('--model_path', default='./conf_model/0604.pth', type=str, help='model path.')
