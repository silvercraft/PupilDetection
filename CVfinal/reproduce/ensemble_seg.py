...
for i,path in enumerate(paths):
    ...
    for dir in sorted(os.listdir(path)):
        for j, file in enumerate(sorted(os.listdir(os.path.join(path, dir)))):
            if file[-3:]=='txt':
                continue
            file_name.append(os.path.join(dir, file))
            img = np.array(cv2.imread(os.path.join(path, dir, file)), np.int32)
            if i==0:
                imgs.append(img)
            else:
                imgs[idx] += img
...
for i in range(imgs.shape[0]):
    ...
    print(file_name[i])
    os.makedirs(os.path.join(sys.argv[4], file_name[i].split('/')[0]), exist_ok = True)
