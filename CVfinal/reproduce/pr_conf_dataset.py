...
class classify_test(Dataset):
    def __init__(self, root, transform=None):
        ...
        for sdir in sorted(os.listdir(root)):
            for idx, dir_name in enumerate(sorted(os.listdir(os.path.join(root, sdir)))):
                imgs = os.listdir(os.path.join(root, sdir, dir_name))
                # print(root.split('/')[-1])
                ...
                #     num = len(imgs)
                num = len(imgs)
                for name in os.listdir(os.path.join(root, sdir, dir_name)):
                    if 'png' in name:
                        num = len(imgs)//2
                        break
                print(f'num: {num}')
                for img_name in range(num):
                    filename = os.path.join(root, sdir, dir_name, str(img_name)+'.jpg')
