...
class testDataset(Dataset):
    ...
    def __init__(self, root_dir, feature_extractor):
        ...
        # read images
        image_file_names = []
        for sdir in os.listdir(self.root_dir):
            for dir in os.listdir(os.path.join(self.root_dir, sdir)):
                for name in os.listdir(os.path.join(self.root_dir, sdir, dir)):
                    if 'jpg' in name:
                        image_file_names.append(os.path.join(self.root_dir, sdir, dir, name))
...
parser.add_argument('--img_dir', type=str, default='./public/S5')
...
#   print(image.shape)
  ...
  with torch.no_grad():
    ...
    #   print(img_fn[i])
      ...
      # plt.show()
      dir_name = '/'.join(img_fn[i].split('/')[2:-1])
      print(dir_name)
      os.makedirs(os.path.join(config.save_path, dir_name), exist_ok = True)
      cv2.imwrite(os.path.join(config.save_path, '/'.join(img_fn[i].split('/')[2:])[:-3]+'png'), color_seg)