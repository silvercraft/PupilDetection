...
def benchmark(dataset_path: str, result_path: str, subjects: list):
    ...
    for subject in subjects:
        for action_number in range(26):
            ...
            for idx in tqdm(range(nr_image), desc=f'[{sequence_idx:03d}] {image_folder}'):
                ...
                output_name = os.path.join(result_folder, f'{idx}.png')
...
def conf_benchmark(dataset_path: str, result_path: str, subjects: list):
    """Compute the weighted IoU and average true negative rate
    Args:
        dataset_path: the dataset path
        subjects: a list of subject names
    ...
    for subject in subjects:
        for action_number in range(26):
            ...
            result_conf = os.path.join(result_path, subject, f'{action_number + 1:02d}', 'conf.txt')
            with open(result_conf, 'r') as f:
                conf_txt = f.read().splitlines()
...
if __name__ == '__main__':
    ...
    # result_path = './reproduce/ensemble_public_fit'
    result_path = './yolo_seg'
    subjects = ['S5']

    benchmark(dataset_path, result_path, subjects)

    result_path = './yolo_conf'
    # conf_benchmark(dataset_path, result_path, subjects)